...
use rs_cp_core::{DeviceId, TrustState, broadcast_targets};
...
fn trust_device(args: Vec<String>) -> io::Result<()> {
    if args.len() < 3 || args.len() > 4 {
        println!("usage: rs-cp-daemon trust <id> <name> <platform> [host]");
        return Ok(());
    }
    ...
    store.trust(id, name.clone(), platform, endpoint.clone(), None)?;
    println!(
        "trusted: {name} ({}){}",
...
fn revoke_device(id: Option<String>) -> io::Result<()> {
    let Some(id) = id.and_then(DeviceId::new) else {
        println!("usage: rs-cp-daemon revoke <id>");
        return Ok(());
    };
...
fn discover_devices() -> io::Result<()> {
    println!("listening for rs-cp devices for 2s...");
    for (packet, addr) in listen_once(Duration::from_secs(2))? {
        let info = packet.into_device_info();
        println!(
            "- {}\t{}\t{}\t{}",
            info.id,
            info.name,
...
fn write_clipboard(args: Vec<String>) -> io::Result<()> {
    if args.is_empty() {
        println!("usage: rs-cp-daemon clip-write <text>");
        return Ok(());
    }
...
fn send_text_command(args: Vec<String>) -> io::Result<()> {
    let [host, text @ ..] = args.as_slice() else {
        println!("usage: rs-cp-daemon send-text <host> <text>");
        return Ok(());
    };
